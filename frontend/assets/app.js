function stateBase() {
  ...
  const idx = p.indexOf("/state/");
  if (idx === -1) {
    const root = p.endsWith("/") ? p : p.replace(/[^/]+$/, "");
    return root + (root.endsWith("/") ? "" : "/") + "state/";
  }
  return p.slice(0, idx + "/state/".length);
}

function pathTo(route) {
  return stateBase() + String(route).replace(/^\//, "");
}
...
function bindSearch() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const onActs = /\/state\/acts\/?$/.test(location.pathname.replace(/index\.html$/, ""));
    if (onActs) {
...
function markActiveNav() {
  ...
  document.querySelectorAll(".nav a[href]").forEach((a) => {
    ...
    try {
      ...
      if (here === path || (path.endsWith("/main/") && /\/state\/main\/?$/.test(here))) {
